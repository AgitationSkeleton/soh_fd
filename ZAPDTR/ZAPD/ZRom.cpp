...
ZRom::ZRom(std::string romPath)
{
	...
	// FD (2026-07-16): the yar-compressed archive indices below are MM-specific. Decide at runtime from the
	// already-detected ROM version rather than the old compile-time GAME_MM: this ZAPD is built for OoT (so
	// GAME_MM is undefined), but it must still extract MM assets when generating fd.o2r.
	const bool isMM = (version.crc == MM_NTSC_10 || version.crc == MM_NTSC_10_UNCOMPRESSED ||
	...
	for (unsigned int i = 0; i < lines.size(); i++)
	{
		...
		// FD (2026-07-16): runtime MM check (was #ifdef GAME_MM). These DMA indices are yar-compressed
		// archives in Majora's Mask (icon_item_static, etc.).
		if (isMM && ((i >= 15 && i <= 20) || i == 22))
		{
			yarCompressed = true;
		}
