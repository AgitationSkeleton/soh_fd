...
Declaration* ZPlayerAnimationData::DeclareVar(const std::string& prefix, const std::string& bodyStr)
{
	...
	// FD (2026-07-16): AddDeclarationArray returns null for an out-of-file-range offset (an intentional
	// "ignore" case). Guard the deref, which otherwise access-violates when this MM PlayerAnimationData is
	// declared from an external file (e.g. gameplay_keep referencing link_animetion during fd.o2r generation).
	if (decl != nullptr)
