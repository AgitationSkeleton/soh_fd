...
f32 Player_GetHeight(Player* player) {
    ...
    // adult's 68 * (0.015/0.01) = ~102, not 124. The old 124 aimed the camera ~22 units ABOVE his real head, which
    // read fine for a distant Z-target/Navi shot but framed the get-item way too high (ceiling above, hands + head
    // cut off). 102 matches his on-screen head so the get-item frames his hands + head-top like adult/child, and
    // Z-target/Navi still lock onto his (now correctly-located) head. Must stay non-zero -- 0 = Camera_Normal1 FPE.
    if (LINK_IS_DEITY) {
