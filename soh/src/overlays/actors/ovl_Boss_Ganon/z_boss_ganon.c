...
void BossGanon_Update(Actor* thisx, PlayState* play2) {
    ...
    // FD (2026-07-12) BOSS PARITY (aegiker): re-enable Z-targeting on Ganon for Fierce Deity (he clears his own
    // attention flag above every frame) so FD can lock on and aim his sword beams at him. RE Boss_Ganon
    // z_boss_ganon.c:2833 (ACTOR_FLAG_0 == SoH ACTOR_FLAG_ATTENTION_ENABLED). Targeting only -- no beam-damage path.
    if (LINK_IS_DEITY) {
        this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
    }
