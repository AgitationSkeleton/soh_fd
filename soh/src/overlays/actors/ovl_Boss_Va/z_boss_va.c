...
void BossVa_BodyPhase2(BossVa* this, PlayState* play) {
    ...
    // FD (2026-07-12) BOSS PARITY (aegiker): a Fierce Deity great sword beam near Barinade's core substitutes for
    // the boomerang stun (the RE added DMG_SWORD_BEAM to this boomerang-only mask). Our beam is sword-tier (0x200),
    // so instead of widening the 0x10 mask -- which would also admit a plain sword slash -- scan for the beam actor
    // near the core and trigger the SAME stun the boomerang does (open the core, swap to the 0xFC00712 mask that
    // already accepts the beam's 0x200 so FD can then damage it). RE Boss_Va z_boss_va.c:1132.
