...
s32 BossDodongo_AteExplosive(BossDodongo* this, PlayState* play) {
    ...
    // FD (2026-07-12) BOSS PARITY (aegiker): King Dodongo swallows a Fierce Deity great sword beam near his open
    // mouth exactly like a bomb (the hack's composite beam carries DMG_EXPLOSIVE; here we detect the beam actor).
    // RE BossDodongo_AteExplosive z_boss_dodongo.c:177 -- a second scan of the item-action list for a great beam
    // within +-40 of mouthPos; on a hit, kill the beam and report "ate explosive" so the normal swallow -> internal
    // explosion damage runs.
    {
