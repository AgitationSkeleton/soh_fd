...
ItemObtainability Randomizer::GetItemObtainabilityFromRandomizerGet(RandomizerGet randoGet) {
    ...
    switch (randoGet) {
        ...
        case RG_FIERCE_DEITY_MASK:
            return Flags_GetRandomizerInf(RAND_INF_OBTAINED_FIERCE_DEITY_MASK) ? CANT_OBTAIN_ALREADY_HAVE : CAN_OBTAIN;
...
extern "C" u16 Randomizer_Item_Give(PlayState* play, GetItemEntry giEntry) {
    ...
    switch (item) {
        ...
        case RG_ROCS_FEATHER:
            ...
            break;
        case RG_FIERCE_DEITY_MASK:
            // FD (2026-07-12) #6: grant the FD mask. Item_Give(ITEM_MASK_DEITY) only marks it obtained
            // (gSaveContext.ship.hasFierceDeityMask = 1, z_parameter.c:1945) -- it does NOT auto-equip to a
            // C-button. The player equips it themselves via the kaleido bottle-slot cycle, same as the
            // non-rando obtainable mask.
            Flags_SetRandomizerInf(RAND_INF_OBTAINED_FIERCE_DEITY_MASK);
            Item_Give(play, ITEM_MASK_DEITY);
            break;
        default:
