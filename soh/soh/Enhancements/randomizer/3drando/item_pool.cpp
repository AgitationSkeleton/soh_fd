...
void GenerateItemPool() {
    ...
    // FD (2026-07-12) #6: FD mask uses the SAME weight as Biggoron's Sword (2,1,1,0) -- as Roc's matches Nayru's.
    // FD (2026-07-12): "Start with Fierce Deity's Mask" removes it from the shuffled pool (like Start-with-Kokiri-
    // Sword at ~339) -- the player already has it, so it must not also be placed as a check.
    if (ctx->GetOption(RSK_FIERCE_DEITY_MASK) && !ctx->GetOption(RSK_STARTING_FIERCE_DEITY_MASK)) {
        AddItemToPool(RG_FIERCE_DEITY_MASK, 2, 1, 1, 0);
    }
