...
void SetStartingItems() {
    ...
    // FD (2026-07-12): "Start with Fierce Deity's Mask". Item_Give(ITEM_MASK_DEITY) sets
    // gSaveContext.ship.hasFierceDeityMask (z_parameter.c) -- the ownership flag the kaleido bottle-slot cycle and
    // the item tracker key off. Also set the RandomizerInf so the rando tracker reflects it as obtained.
    if (Randomizer_GetSettingValue(RSK_STARTING_FIERCE_DEITY_MASK)) {
        Item_Give(NULL, ITEM_MASK_DEITY);
