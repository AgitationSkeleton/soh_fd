...
static void ExporterProgramEnd()
{
    ...
    if (Globals::Instance->fileMode == ZFileMode::ExtractDirectory)
    {
        ...
        // FD (2026-07-16): IsDirectory, not Exists (which is true for files too) - otherwise the ROM crc for the
        // archive's version file is never read when -b points at a ROM file, and the engine rejects the archive.
        if (!Directory::IsDirectory(romPath)) {
